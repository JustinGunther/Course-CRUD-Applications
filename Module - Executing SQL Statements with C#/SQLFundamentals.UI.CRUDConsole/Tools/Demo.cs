...
namespace SQLFundamentals.UI.CRUDConsole.Tools
{
    public class Demo
    {
        private ContactController contactController = new();
        ...
        private void DeleteContact()
        {
            ...
            string optionNo = Console.ReadLine() ?? "1";
            switch (optionNo)
            {
                case "1":
                    Console.Write("\nEnter Contact ID:");
        ...
        private void GetAllContacts()
        {
            try
            {
                List<ContactModel> contactModels = contactController.GetAllContacts() ?? new();
                Console.WriteLine(
        ...
        private void GetContactByID()
        {
            try
            {
                Console.Write("\nEnter Contact ID:");
                int contactID = Convert.ToInt32(Console.ReadLine());
                ContactModel contactModel = contactController.GetContactByID(contactID) ?? new();
                Console.WriteLine(
                    String.Format("|{0,10}|{1,15}|{2,15}|{3,20}|{4,25}|", "Contact ID", "First Name", "Last Name", "Phone Number", "Email Address")
                    );
